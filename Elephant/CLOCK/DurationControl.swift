...
class DurationControl: UIControl {
    ...
    private var knobHeight: CGFloat = 30.0
    private var leftLabel: UILabel?
    private var rightLabel: UILabel?
    private var valueLabel: UILabel?
    private var spot:CGFloat = 0
    ...
    var value: TimeInterval {
        get {
            ...
            return TimeInterval(1 + range/(space/spots))
 
        }
        set {
            spot = CGFloat(newValue)
            setNeedsDisplay()
        }
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        let space = self.bounds.width - offSet * 2 - knobHeight
        position = offSet + (space/120) * CGFloat(spot)
        ...
        styles[NSAttributedStringKey.font] = UIFont(name: "DINCondensed-Bold", size: 8 )
